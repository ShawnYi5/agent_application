...
class Mount(object):
    ...
    def __init__(self, nbds, storage, mount_dir, read_only, include_ranges):
        ...
        self.mnt_opt = 'rw,async,relatime,noatime,nodiratime,nosuid'
        self.rmv_opt = 'ro'
        self._uuid2device = dict()  # {'uuid':'device'}
    ...
    def analyze_mount_params(self):
        ...
        for disk in self._storage["disks"]:
            ...
            for partition in disk["partitions"]:
                partition_device_path = nbd_device if (partition["device"] == disk["device"]) else \
                    r'{}{}'.format(nbd_device, partition["index"])
        ...
        for vg in self._storage["vgs"]:
            for lv in vg["lvs"]:
                ...
                # if mount_point_in_snapshot and (
                #         (not self._read_only) or mount_point_in_snapshot in include_mount_points):
                if mount_point_in_snapshot:  # 不再判断是否再include_mount_points 中 #4153
                    mount_params.append({
                        'mountPoint': self.generate_mount_point(mount_point_in_snapshot),
        ...
        for mount_info in self._storage.get('mount_info', list()):
            device = self._uuid2device.get(mount_info['uuid'])
            if not device:
                _logger.warning('get device failed {}'.format(mount_info))
                continue
            mount_params.append({
                'mountPoint': self.generate_mount_point(mount_info['mountpoint']),
                'device': device,
                'fileSystem': mount_info["fstype"],
                'btrfsmntopt': mount_info.get("btrfsmntopt"),
                'orgmntpoint': mount_info['mountpoint']
            })
    ...
    def _scan_device(self):
        ...
        for line in rev[1].strip().splitlines():
            ...
            if uuid_match:
                ...
                pass
            device = line.split(':')[0]
            self._uuid2device[uuid] = device
        _logger.info('_scan_device _uuid2device {}'.format(self._uuid2device))
    ...
    def mount_btrfs_subvol(self, mnttask):
        ...
        for mntopt in btrfsmntopts:
            ...
            if full in self.mount_point:
                _logger.warning("[mount_btrfs_subvol] {} is mounted")
                continue
    ...
    def _pause_when_vgrename_unsuccessful(r):
        ...
        xlogging.makedirs(flag_path, exist_ok=True)
    ...
    def change_xfs_uuid(mount_params):
        for mount_param in mount_params:
            ...
            xlogging.makedirs(_tmp_mount_path, exist_ok=True)
    ...
    def change_chmod_only_read(self):
        _get_info_from_syscmd_timeout_middle(r'chmod -R a-w,a+rx "{}"'.format(self._mount_dir))
